...
import java.nio.charset.Charset; // ← (6) UseStandardCharset の対象
...
public class LegacyResources {
  ...
  public static String firstLine(Path path) throws IOException {
    try (BufferedReader br = new BufferedReader(new FileReader(path.toFile()))) {
      ...
      br.close(); // ← (2) UnnecessaryCloseInTryWithResources で削除される
      return line;
    }
  }
  ...
  public static byte[] toBytes(String s) {
    return s.getBytes(Charset.forName("UTF-8")); // ← (6) UseStandardCharset
  }

  // 冗長なジェネリクス（→ ダイヤモンド演算子）
  public static Map<String, Integer> ranks() {
    Map<String, Integer> map = new HashMap<String, Integer>(); // ← (7) UseDiamondOperator
    map.put("Gold", 1);
    map.put("Silver", 2);
    map.put("Bronze", 3);
    return map;
